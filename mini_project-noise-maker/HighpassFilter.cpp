...
void HighpassFilter::initialise(float cutoff, float sampleRateIn) {
...
float HighpassFilter::process(float x, float dryGain, float wetGain) {
...
void HighpassFilter::update(float newCutoff, float newQ) {
	// recalculate filter
	float q = newQ;
