...
Alongside the Bela itself, it requires a way of listening to the audio output via an audio adapter using the 3 pin Molex-style connector on the Bela.  <br>
...
The programming is comprised of multiple classes handing different parts of audio processing, which are implemented in the main *render.cpp* file. In conjunction with one another, these classes are able to create playable, varied and dynamic noise from an audio file. <br>
...
Together the short feedback loop and comb filter resonances produce outputs, such as instant or near instant feedback and resonant frequencies, similar to those encountered in no-input mixing. <br>
...
The final hard clipper exists primarily as a safety feature, keeping the output signal's amplitude within the range of -1.0 to 1.0. Secondarily serving as a source for further harsh distortion.  <br>
